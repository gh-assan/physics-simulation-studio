...
describe("ComponentManager", () => {
  ...
  test("should add and get components", () => {
    ...
    const retrievedComponent1 = componentManager.getComponent<TestComponent1>(
      entityId,
      TestComponent1.type
    );
    const retrievedComponent2 = componentManager.getComponent<TestComponent2>(
      entityId,
      TestComponent2.type
    );
  ...
  test("should remove components", () => {
    ...
    expect(componentManager.hasComponent(entityId, TestComponent1.type)).toBe(
      true
    );
  ...
  test("should get entities with components", () => {
    ...
    const entitiesWithBoth = componentManager.getEntitiesWithComponents([
      TestComponent1,
      TestComponent2
    ]);
    const entitiesWithComponent1 =
      componentManager.getEntitiesWithComponentTypes([TestComponent1.type]);
    const entitiesWithComponent2 =
      componentManager.getEntitiesWithComponentTypes([TestComponent2.type]);
  ...
  test("should perform batch operations", () => {
    ...
    componentManager.batchOperation(entities, (entityId) => {
      componentManager.addComponent(
        entityId,
        TestComponent1.type,
        new TestComponent1(entityId)
      );
    });

    for (const entityId of entities) {
      const component = componentManager.getComponent<TestComponent1>(
        entityId,
        TestComponent1.type
      );
  ...
  test("should add multiple components at once", () => {
    ...
    expect(componentManager.hasComponent(entityId, TestComponent2.type)).toBe(
      true
    );
  });
