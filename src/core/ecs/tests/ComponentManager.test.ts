...
describe('ComponentManager', () => {
    ...
    it('should add and get a component', () => {
        ...
        const retrieved = componentManager.getComponent<TestComponent>(0, TestComponent.name);
    ...
    it('should get entities with components', () => {
        ...
        componentManager.addComponent(1, AnotherComponent.name, new AnotherComponent("test"));
    ...
    it('should clear all components', () => {
        componentManager.addComponent(0, TestComponent.name, new TestComponent(1));
        componentManager.addComponent(1, AnotherComponent.name, new AnotherComponent("test"));
        expect(componentManager.getAllComponentsForEntity(0)[TestComponent.name]).toBeDefined();
        expect(componentManager.getAllComponentsForEntity(1)[AnotherComponent.name]).toBeDefined();

        componentManager.clear();

        expect(componentManager.getAllComponentsForEntity(0)[TestComponent.name]).toBeUndefined();
        expect(componentManager.getAllComponentsForEntity(1)[AnotherComponent.name]).toBeUndefined();
    });
});