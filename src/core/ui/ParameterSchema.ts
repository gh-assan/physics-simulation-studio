...
// Plugin inference registry - can be configured by plugins at runtime
const PLUGIN_INFERENCE_PATTERNS = new Map<string, string[]>([
  ['flag-simulation', ['flag', 'pole']],
  ['water-simulation', ['water', 'droplet']],
  ['solar-system', ['solar', 'celestial']],
  ['rigid-body-physics', ['rigid', 'physics']]
]);
...
function inferPluginFromComponentType(componentType: string): string | undefined {
  ...
  for (const [pluginId, patterns] of PLUGIN_INFERENCE_PATTERNS.entries()) {
...
// Allow plugins to register their own inference patterns
export function registerPluginInferencePattern(pluginId: string, patterns: string[]): void {
  PLUGIN_INFERENCE_PATTERNS.set(pluginId, patterns);
}
