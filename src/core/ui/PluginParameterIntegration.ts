...
export class ModernPropertyInspectorUIManager {
  ...
  private inferPluginId(componentType: string): string {
    ...
    // Use the same configurable inference system from ParameterSchema
    ...
    for (const [pluginId, patternList] of patterns.entries()) {
      if (patternList.some(pattern => lowerType.includes(pattern))) {
        return pluginId;
      }
    }
