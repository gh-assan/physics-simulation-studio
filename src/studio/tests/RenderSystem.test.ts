...
const mockRenderer = { setSize: jest.fn(), render: jest.fn(), domElement: document.createElement('canvas') };
...
describe('RenderSystem', () => {
    ...
    it('should create and update meshes for entities with renderable components', () => {
        ...
        world.componentManager.addComponent(entity, RotationComponent.name, rotation);
        world.componentManager.addComponent(entity, RenderableComponent.name, renderable);
        ...
        expect(meshInstance.position.set).toHaveBeenCalledWith(position.x, position.y, position.z);
        expect(meshInstance.rotation.setFromQuaternion).toHaveBeenCalledWith(
            expect.objectContaining({ x: rotation.x, y: rotation.y, z: rotation.z, w: rotation.w })
        );
