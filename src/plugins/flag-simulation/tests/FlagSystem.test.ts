...
describe('FlagSystem', () => {
    ...
    it('should initialize flag points and springs when a FlagComponent is added', () => {
        ...
        world.componentManager.addComponent(entity, FlagComponent.name, flagComponent);
        world.componentManager.addComponent(entity, PositionComponent.name, positionComponent);

        flagSystem.update(world, 0);

        expect(flagComponent.points).toBeDefined();
        expect(flagComponent.points.length).toBe((flagComponent.segmentsX + 1) * (flagComponent.segmentsY + 1));
        ...
        const bottomLeft = flagComponent.points[flagComponent.segmentsY * (flagComponent.segmentsX + 1)];
    ...
    it('should apply gravity and wind forces to non-fixed points', () => {
        ...
        const initialForces = { ...nonFixedPoint.forces }; // Should be {x:0, y:0, z:0}

        flagSystem.update(world, 0.1); // Apply forces and integrate

        // Expect forces to have changed due to gravity and wind
        expect(nonFixedPoint.forces.y).toBeCloseTo(flagSystem['gravity'].y * nonFixedPoint.mass); // Gravity should pull down
    });
    ...
    it('should integrate point mass positions using Verlet integration', () => {
        ...
        const nonFixedPoint = flagComponent.points[1]; // A non-fixed point
        const initialPosition = { ...nonFixedPoint.position }; // Define initialPosition here
        ...
        expect(nonFixedPoint.position.y).toBeLessThan(initialPosition.y);
    });
    ...
    it('should satisfy constraints and maintain spring rest lengths (approximately)', () => {
        ...
        const initialPosition = { ...displacedPoint.position };
        ...
        const springToCheck = flagComponent.springs.find(s =>
            (s.p1 === flagComponent.points[0] && s.p2 === flagComponent.points[1]) ||
