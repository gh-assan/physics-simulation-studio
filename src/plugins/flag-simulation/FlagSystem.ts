...
export class FlagSystem extends System {
    ...
    update(world: World, deltaTime: number): void {
        ...
        for (const entityId of flagEntities) {
            const flagComponent = world.componentManager.getComponent(entityId, FlagComponent.name) as FlagComponent;
    ...
    private initializeFlag(flagComponent: FlagComponent, positionComponent: PositionComponent): void {
        ...
        for (let y = 0; y <= flagComponent.segmentsY; y++) {
            for (let x = 0; x <= flagComponent.segmentsX; x++) {
                const initialPoint = flagComponent.initialPoints[y * (flagComponent.segmentsX + 1) + x];
                const pointMass: PointMass = {
                    ...
                    isFixed: (x === 0 && y === 0) || (x === 0 && y === flagComponent.segmentsY) // Fix top-left and bottom-left corners
                };
        ...
        for (let y = 0; y <= flagComponent.segmentsY; y++) {
            for (let x = 0; x <= flagComponent.segmentsX; x++) {
                ...
                if (x < flagComponent.segmentsX) {
                    ...
                    flagComponent.springs.push(this.createSpring(p1, p2, 'structural', segmentWidth, flagComponent.stiffness, flagComponent.damping));
                }
                if (y < flagComponent.segmentsY) {
                    const p2 = flagComponent.points[p1Index + (flagComponent.segmentsX + 1)];
                    flagComponent.springs.push(this.createSpring(p1, p2, 'structural', segmentHeight, flagComponent.stiffness, flagComponent.damping));
                }

                // Shear springs (diagonal)
                if (x < flagComponent.segmentsX && y < flagComponent.segmentsY) {
                    const p2 = flagComponent.points[p1Index + 1 + (flagComponent.segmentsX + 1)];
                    flagComponent.springs.push(this.createSpring(p1, p2, 'shear', Math.sqrt(segmentWidth * segmentWidth + segmentHeight * segmentHeight), flagComponent.stiffness, flagComponent.damping));

                    const p3 = flagComponent.points[p1Index + (flagComponent.segmentsX + 1) - 1];
                    if (x > 0) {
                        flagComponent.springs.push(this.createSpring(p1, p3, 'shear', Math.sqrt(segmentWidth * segmentWidth + segmentHeight * segmentHeight), flagComponent.stiffness, flagComponent.damping));
                    }
                }
                ...
                if (x < flagComponent.segmentsX - 1) {
                    ...
                    flagComponent.springs.push(this.createSpring(p1, p2, 'bend', segmentWidth * 2, flagComponent.stiffness, flagComponent.damping));
                }
                if (y < flagComponent.segmentsY - 1) {
                    const p2 = flagComponent.points[p1Index + 2 * (flagComponent.segmentsX + 1)];
                    flagComponent.springs.push(this.createSpring(p1, p2, 'bend', segmentHeight * 2, flagComponent.stiffness, flagComponent.damping));
                }
            }
        }
    }

    private createSpring(p1: PointMass, p2: PointMass, type: Spring['type'], restLength: number, stiffness: number, damping: number): Spring {
    ...
    private applyForces(flagComponent: FlagComponent): void {
        ...
        for (const spring of flagComponent.springs) {
            ...
            const dampingForce = spring.damping * (dvx * dx + dvy * dy + dvz * dz) / distance;
    ...
    private integrate(flagComponent: FlagComponent, deltaTime: number): void {
        ...
        for (const point of flagComponent.points) {
            ...
            const nextPositionX = point.position.x + (point.position.x - point.previousPosition.x) + accelerationX * dtSq;
            const nextPositionY = point.position.y + (point.position.y - point.previousPosition.y) + accelerationY * dtSq;
            const nextPositionZ = point.position.z + (point.position.z - point.previousPosition.z) + accelerationZ * dtSq;

            point.velocity.x = (nextPositionX - point.previousPosition.x) / (2 * deltaTime);
            point.velocity.y = (nextPositionY - point.previousPosition.y) / (2 * deltaTime);
            point.velocity.z = (nextPositionZ - point.previousPosition.z) / (2 * deltaTime);
    ...
    private updatePositions(flagComponent: FlagComponent, positionComponent: PositionComponent): void {
