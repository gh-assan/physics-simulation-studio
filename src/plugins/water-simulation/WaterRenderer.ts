...
export class WaterRenderer {
    ...
    render(world: World, scene: THREE.Scene, camera: THREE.Camera): void {
        const waterBodies = world.componentManager.getEntitiesWithComponents([WaterBodyComponent, PositionComponent, RenderableComponent]);
        ...
        const waterBodyRenderable = world.componentManager.getComponent(waterBodyEntity, RenderableComponent.name) as RenderableComponent;
        ...
        const waterMesh = scene.children.find(child => {
            ...
            return (child instanceof THREE.Mesh &&
                    ...
                    (child.material as THREE.MeshBasicMaterial).color.getHex() === new THREE.Color(waterBodyRenderable.color).getHex());
        }) as THREE.Mesh;

        if (!waterMesh) {
            console.warn("WaterRenderer: Could not find water body mesh in scene. Attempting to create a temporary one for debugging.");
            ...
            const material = new THREE.MeshBasicMaterial({ color: 0x0066ff, transparent: true, opacity: 0.8, side: THREE.DoubleSide });
            const tempWaterMesh = new THREE.Mesh(geometry, material);
            tempWaterMesh.rotation.x = -Math.PI / 2;
            tempWaterMesh.position.set(waterBodyPosition.x, waterBodyPosition.y, waterBodyPosition.z);
        ...
        scene.children = scene.children.filter(child => !child.name.startsWith('ripple_'));

        waterBodyComponent.ripples.forEach((ripple, index) => {
            const rippleGeometry = new THREE.RingGeometry(ripple.radius * 0.8, ripple.radius, 32);
            const rippleMaterial = new THREE.MeshBasicMaterial({ color: 0xffffff, transparent: true, opacity: ripple.amplitude, side: THREE.DoubleSide });
