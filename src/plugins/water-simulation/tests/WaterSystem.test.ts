...
describe('WaterSystem', () => {
    ...
    it('should make a droplet fall over time', () => {
        ...
        world.componentManager.addComponent(droplet, WaterDropletComponent.type, new WaterDropletComponent(0.5, 10, 0));
        ...
        const initialPosition = (world.componentManager.getComponent(droplet, PositionComponent.name) as PositionComponent).y;

        // Simulate one second of time
        waterSystem.update(world, 1);

        const finalPosition = (world.componentManager.getComponent(droplet, PositionComponent.name) as PositionComponent).y;
    ...
    it('should remove the droplet when it hits the water', () => {
        const droplet = world.entityManager.createEntity();
        world.componentManager.addComponent(droplet, WaterDropletComponent.type, new WaterDropletComponent(0.5, 1, 0));
        world.componentManager.addComponent(droplet, PositionComponent.name, new PositionComponent(0, 0.1, 0));
        ...
        const entitiesWithDroplet = world.componentManager.getEntitiesWithComponents([WaterDropletComponent]);
